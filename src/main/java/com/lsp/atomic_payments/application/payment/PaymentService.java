            ...
            .flatMap(
                tuple -> {
                  ...
                  if (fromAccount.balance().amount().compareTo(command.amount().amount()) < 0) {
                    return Mono.error(
                        new InsufficientFundsException(fromAccount.accountId(), command.amount()));
                  }

                  Payment payment = Payment.initiate(command);

                  LedgerPair ledgerPair =
                      LedgerEntry.createLedgerPair(
                  ...
                  return paymentRepository
                      ...
                      .thenMany(
                          ledgerRepository.saveAll(
                              List.of(ledgerPair.credit(), ledgerPair.debit())))
                      .then(accountRepository.update(fromUpdated))
                      .then(accountRepository.update(toUpdated))
                      .thenReturn(payment);
                });

    return logic.onErrorMap(this::translateConcurrencyError);
  }
