...
public class LedgerEntryRepositoryImpl implements LedgerRepository {
    ...
    public Flux<LedgerEntry> findByAccountId(AccountId accountId) {
        ...
        return ledgerEntryR2dbcRepository.findByAccountIdOrderByCreatedAtDesc(accountId.value()).map(mapper::toDomain);
    }

    @Override
    public Flux<LedgerEntry> findByPaymentId(PaymentId paymentId) {

        return ledgerEntryR2dbcRepository.findByPaymentIdOrderByCreatedAtDesc(paymentId.value()).map(mapper::toDomain);
    }
