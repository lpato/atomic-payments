...
class LedgerEntryRepositoryImplTest {
  ...
  @Autowired private LedgerRepository ledgerEntryRepository;
        ...
        accountRepository
            ...
            .then(
                accountRepository
                    ...
                    .thenMany(ledgerEntryRepository.saveAll(entries))
                    .then());

    Flux<LedgerEntry> result =
        setUp.thenMany(ledgerEntryRepository.findByPaymentId(payment.paymentId()));
        ...
        .consumeRecordedWith(
            found -> {
              ...
              assertThat(found)
                  ...
                  .contains(ledger1.ledgerEntryId(), ledger2.ledgerEntryId());
            })
        .verifyComplete();
  }
}
